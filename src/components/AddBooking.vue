    ...
    <v-dialog v-model="getDialog" persistent max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="createBooking">Salvar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
    ...
    createBooking() {
      ...
      this.$store
        ...
        .catch(err => {
          ...
          this.errorMessage = 'Erro ao adicionar reserva, verifique os dados e tente novamente'
        });
    }
  },
  mounted() {
      this.booking.rg = this.costumerRg;
      this.booking.name = this.costumerName;
      this.booking.phone_number = this.costumerPhoneNumber;
  }
};
</script>